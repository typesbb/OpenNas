...
- **灵活的照片管理** — 新建、重命名、删除相册；上传照片、多选移动或删除，按时间 / 名称 / 大小排序浏览（排序偏好会自动记住）
...
- **长按相册** 可 **重命名** 或 **删除**（删除不可恢复）
- 进入相册后可按 **拍摄时间**、**文件名称**、**文件大小** 排序，照片分组展示；排序方式会记住，下次打开仍生效
- 点击照片全屏查看原图：双指缩放、左右滑动切换、下滑关闭
- 视频可直接播放；Android 上支持画中画小窗
- **长按照片** 进入多选，可批量 **移动** 到其他相册或 **删除**
